...
The `GenVidIM` project is organized as follows:

- **`wan/`**: Contains the core source code for the video generation models, including modules for different tasks like text-to-video, image-to-video, and animation.
- **`serverless/`**: Includes files for deploying the models on serverless platforms like RunPod, including Dockerfiles and handlers.
- **`examples/`**: A collection of sample inputs (images, audio files) to test the models.
- **`assets/`**: Contains static assets like logos and images used in the documentation.
- **`generate.py`**: The main script for running video generation tasks.
- **`config.py`**: A centralized configuration file for all project settings.
- **`setup.py`**: A setup script to automate the installation of the project and its dependencies.

## Automated Installation

To streamline the setup process, you can now install the project and its dependencies with a single command. This will handle all the requirements automatically, so you can get started with generating videos right away.

```bash
pip install .
```

For specific tasks, you can install the required extras:

```bash
# For animation tasks
pip install .[animate]

# For speech-to-video tasks
pip install .[s2v]
...
The project now uses a centralized configuration file, `config.py`, located in the `GenVidIM` directory. This file contains all the settings for the project, including model paths, generation parameters, and API keys. By consolidating all the settings in one place, it's now easier to manage them across different environments.

## How to Contribute

We welcome contributions from the community! If you'd like to contribute to the project, please follow these steps:

1.  **Fork the repository:** Create your own fork of the project to get started.
2.  **Create a new branch:** Make your changes in a dedicated branch to keep them organized.
3.  **Commit your changes:** Follow the existing coding style and commit your changes with clear, descriptive messages.
4.  **Submit a pull request:** Once your changes are ready, submit a pull request so we can review your contributions.
